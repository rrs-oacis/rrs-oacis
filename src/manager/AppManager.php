...
class AppManager
{
	...
	public static function getApps()
	{
		...
		foreach ($userFiles as $userFile) {
			...
			if (!is_dir(Config::$SRC_REAL_URL . self::APPS_DIR . "/" . $userFile)) {
				continue;
			}
			...
			foreach ($appFiles as $appFile) {
				...
				if (!is_dir(Config::$SRC_REAL_URL . self::APPS_DIR . "/" . $userFile . "/" . $appFile)) {
					continue;
				}
	...
	private static function connectDB()
	{
		...
		switch ($connectedAppVersion) {
			...
			case 1:
				$db->query("update connectedApp set package = 'rrs_oacis/' || package;");
				exec("find /home/oacis/rrs-oacis/data -name '*.db' -type f ! -name '_main.db' ! -name '*@*.db' | sed -E 's/\.db$//' | xargs -I { mv {.db {@rrs_oacis.db");
			case 2:
				$db->query("update connectedApp set package = 'rrs_oacis/archive' where package = 'rrs_oacis/archived';");
				$db->query("update connectedApp set package = 'rrs_oacis/agents_uploader' where package = 'rrs_oacis/agent_multi_upload';");
				$db->query("update connectedApp set package = 'rrs_oacis/maps_uploader' where package = 'rrs_oacis/map_multi_upload';");
			case 3:
				$db->query("delete from connectedApp where package = 'rrs_oacis/manual_post';");
				$db->query("delete from connectedApp where package = 'rrs_oacis/manual_post2';");
				$version = 4;
